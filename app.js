...
function render(){const q=$("#searchInput").value.trim().toLowerCase(),cat=$("#categoryFilter").value;const base=products.filter(p=>p.visible!==false).filter(p=>!q||p.name.toLowerCase().includes(q)||(p.description||"").toLowerCase().includes(q)).filter(p=>!cat||p.category===cat);$("#newGrid").innerHTML=base.filter(p=>p.isNew).slice(0,8).map(productCard).join("");$("#popularGrid").innerHTML=base.filter(p=>p.popular).slice(0,8).map(productCard).join("");$("#featuredGrid").innerHTML=base.filter(p=>p.featured).slice(0,8).map(productCard).join("");let all=base;let title="전체 상품";if(view==="new"){all=base.filter(p=>p.isNew);title="신상품"}if(view==="popular"){all=base.filter(p=>p.popular);title="인기상품"}if(view==="featured"){all=base.filter(p=>p.featured);title="추천상품"}$("#allTitle").textContent=title;$("#allGrid").innerHTML=all.map(productCard).join("");$("#emptyState").classList.toggle("hidden",all.length>0);$("#homeSections").classList.toggle("hidden",view!=="all");attachCards();renderCategories();renderAdminProducts()}
function setView(v){view=v;$$(".tab").forEach(b=>b.classList.toggle("active",b.dataset.view===v));render();$("#allSection").scrollIntoView({behavior:"smooth"});closeDrawer()}
...
$("#menuBtn").onclick=openDrawer;$("#drawerClose").onclick=closeDrawer;$("#overlay").onclick=closeDrawer;$("#searchBtn").onclick=()=>$("#searchPanel").classList.toggle("hidden");$("#searchInput").oninput=render;$("#categoryFilter").onchange=render;
